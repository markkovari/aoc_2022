...
pub fn get_11_first() -> usize {
    let input = include_str!("../../inputs/11/input.data");
    let monkeys = load_monkeys(input).unwrap();

    runner(monkeys.clone(), 20, true, 0)
}
pub fn get_11_second() -> usize {
    let input = include_str!("../../inputs/11/input.data");
    let monkeys = load_monkeys(input).unwrap();
...
mod tests {
    ...
    #[test]
    fn test_example_answers_first() {
        let input = include_str!("../../inputs/11/input.example");
        let monkeys = load_monkeys(input).unwrap();

        let result = runner(monkeys.clone(), 20, true, 0);
        assert_eq!(result, 10605);
    }
    #[test]
    fn test_example_answers_second() {
        let input = include_str!("../../inputs/11/input.example");
        let monkeys = load_monkeys(input).unwrap();
